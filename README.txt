...
This conversion to Java was written by Rock de Vocht @ booktrack.com
...
Java Code EXAMPLE:
    ...
    // read the contents of some text-file
    String fileText = new String(Files.readAllBytes(Paths.get("/path/to/text-file.txt")));

    // setup Vader
    Vader vader = new Vader();
    vader.init(); // load vader from classpath

    // setup nlp processor
    VaderNLP vaderNLP = new VaderNLP();
    vaderNLP.init(); // load open-nlp from classpath

    // parse the text into a set of sentences (a list of sentences, a sentence being a list of Token)
    ...
    for ( List<Token> sentence : sentenceList ) {
        VScore vaderScore = vader.analyseSentence(sentence); // score the sentence
        logger.info("sentence:" + Token.tokenListToString(sentence) ); // print the sentence as English text
        logger.info("Vader score:" + vaderScore.toString()); // output the vader score below the sentence
    }
